/*
{   'A': 1.5878736672977207,
    'B': 0.06081936033711212,
    'C': 0.2090922258863806,
    'D': 1.7007183570349196,
    'X': 0.858270582353355,
    'anchor_distance_ratio': 0.5,
    'anchor_double': True,
    'anchor_num': 16,
    'anchor_number_ratio': 0.6666666666666666,
...
#define POS(X, Y) ((X)*y_size + (Y))
    ...
    if (best_seed == 0)
        ...
        for(int j = -1; j < 1; j++)
        for(int i = 0; i < 8 * (
                ( 1 + j) * (1 - 0.6666666666666666) 
                +
                (-1 * j) * (0.6666666666666666)
            ); i++) {
            int A = (step * cos((float) ((6.28/8) * i) ));
            int B = (step * sin((float) ((6.28/8) * i) ));
            if (j == 0) {
               A *= 0.5;
               B *= 0.5;
            }
