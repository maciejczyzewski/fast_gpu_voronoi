/*
{   'A': 1.4463160262872736,
    'B': 0.9888341658752698,
    'C': 0.23019069194454128,
    'D': 1.7834847053349974,
    'X': 0.9277468621877312,
    'anchor_distance_ratio': 0.3333333333333333,
    'anchor_double': True,
    'anchor_type': <function mod_anchor_type__square at 0x11ee17e60>,
    'bruteforce': False,
    'noise': 'noise',
...
#define POS(X, Y) ((X)*y_size + (Y))
    ...
    if (best_seed == 0)
        ...
        int pos[] = {-step, -step*(0.3333333333333333), 0, step*(0.3333333333333333), step};
        for(int i = 0; i < 5; i++)
            for(int j = 0; j < 5; j++) {
                int nx = x + pos[i];
                int ny = y + pos[j];
