...
#define POS(X, Y, Z) ((X)*y_size*3 + (Y)*3 + Z)
	...
	// FIXME: noise in place
	// FIXME: step options
	// FIXME: fast push forward `anchor_seed == 0` continue;
	// FIXME: squere / circle
	// FIXME: metric weight push
	...
	int pos[] = @{step_extended[pos]};

    for(int i = 0; i < @{step_extended[i]}; i++)
        for(int j = 0; j < @{step_extended[j]}; j++) {
            int nx = x + pos[i];
            int ny = y + pos[j];
            if(nx < 0 || y_size <= nx || ny < 0 || y_size <= ny) continue;
            int anchor_seed = mat2d_in[POS(nx, ny, 0)];
