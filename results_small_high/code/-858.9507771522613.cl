/*
{   'A': 1.1949425649464946,
    'B': 0.08568347222024099,
    'C': 0.6652657554150029,
    'D': 1.3486241204640488,
    'X': 0.4466692008727034,
...
#define POS(X, Y) ((X)*y_size + (Y))
    ...
    if (best_seed == 0)
        ...
        for(int i = 0; i < 14; i++) {
            int A = (step * cos((float) ((6.28/14) * i) ));
            int B = (step * sin((float) ((6.28/14) * i) ));
            int nx = x+A;
            int ny = y+B;
