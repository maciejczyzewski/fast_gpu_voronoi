/*
{   'A': 1.3733147724195376,
    'B': 0.8207782316384271,
    'C': 0.035427399278054945,
    'D': 1.1642165076936077,
    'X': 0.8830925420907434,
...
#define POS(X, Y) ((X)*y_size + (Y))
    ...
    if (best_seed == 0)
        ...
        for(int i = 0; i < 13; i++) {
            int A = (step * cos((float) ((6.28/13) * i) ));
            int B = (step * sin((float) ((6.28/13) * i) ));
            int nx = x+A;
            int ny = y+B;
