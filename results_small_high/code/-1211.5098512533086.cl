/*
{   'A': 1.3104251439792054,
    'B': 0.5194573028251123,
    'C': 0.49441724168590684,
    'D': 1.1489613441117623,
    'X': 0.4649449467995378,
...
#define POS(X, Y) ((X)*y_size + (Y))
    ...
    if (best_seed == 0)
        ...
        for(int i = 0; i < 16; i++) {
            int A = (step * cos((float) ((6.28/16) * i) ));
            int B = (step * sin((float) ((6.28/16) * i) ));
            int nx = x+A;
            int ny = y+B;
