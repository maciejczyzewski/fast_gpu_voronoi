/*
{   'A': 1.4613063776268405,
    'B': 0.7124232004649838,
    'C': 0.4047687921323957,
    'D': 1.2326926779434908,
    'X': 0.5912330752184072,
...
#define POS(X, Y) ((X)*y_size + (Y))
    ...
    if (best_seed == 0)
        ...
        for(int i = 0; i < 11; i++) {
            int A = (step * cos((float) ((6.28/11) * i) ));
            int B = (step * sin((float) ((6.28/11) * i) ));
            int nx = x+A;
            int ny = y+B;
