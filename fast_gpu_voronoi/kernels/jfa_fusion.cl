...
__kernel void fn(
    __global       uint *mat2d_in,
    __global const uint *steps_in,
    int                 x_size,
    int                 y_size,
	int                 steps
) {
...
#define POS(X, Y, Z) ((X)*y_size*3 + (Y)*3 + Z)
	...
	// FIXME: special hardware like TPU will help?
	// FIXME: dodac 4 wymiar (ile razy update)? + last if
	//        + aby poprawic sync. / bariery
	for (int layer = 0; layer < steps; layer++) {
		int step = steps_in[layer];
		...
		if (mat2d_in[POS(best_pos_x, best_pos_y, 0)] == best_seed) {
			mat2d_in[POS(x,y,1)] = best_pos_x;
			mat2d_in[POS(x,y,2)] = best_pos_y;
			mat2d_in[POS(x,y,0)] = best_seed;
		}
