...
%       tak wyglada mieszkanko jak pisze prace w nocy!
...
% Evolving New State-of-the-Art for Distance Transform From JFA Using AutoML
...
% [FIXME]
% - additionally, we present \ourjfasingle, single variant that outperforms JFA in most cases.
...
% We choose this variant because it's the simplest and have wide range of improvements to apply.
...
% (JFA) o dowodzie ze log(n)
% (JFA) o zbednych przekazaniach

% [original]
% 
% "Generalized Voronoi Diagram Computation on GPU"
%
% The jump flooding algorithm (JFA) computes a Voronoi
% diagram in a discrete 2D space represented by a 2D texture.
% It propagates the information (2D coordinates and ID) of all
% sites to all the pixels in the texture. In the first stage, called
% mapping stage, each site is mapped to its corresponding
% pixel in the texture. It can be done in parallel by the GPU
% rasterizer. The second stage, flooding stage, consists of log 𝑛
% rounds of flooding, where 𝑛 is the dimension of the texture.
% All pixels are processed in parallel by the GPU. In each
% round, the site information stored in each pixel 𝑝(𝑥, 𝑦) is
% propagated to at most eight other pixels at (𝑥 + 𝑖, 𝑦 + 𝑗)
% where 𝑖, 𝑗 ∈ {−𝑘, 0, 𝑘}, and 𝑘 is the step length of the
% current round. In the first round, we use 𝑛/2 as the initial
% step length to ensure that each pixel is reached by at least
% one site. Here we assume 𝑛 is a power of 2. If not, the initial
% step length is set as 2⌈log 𝑛⌉−1. The step length 𝑘 is halved
% in each of the following round. When more than one sites
% are propagated into the same pixel, the nearest site to the
% pixel will be chosen to update it. After the round with the
% step length of 1, the result is the computed Voronoi diagram.
...
%      https://arxiv.org/pdf/1711.03243.pdf
% --> "Enhancing Dynamic Symbolic Execution by
%             Automatically Learning Search Heuristics"
%      https://arxiv.org/pdf/1907.09700.pdf
...
% Another factor is how efficient the generated code is if we have wide range
% acceptable solution.

% [FIXME]
% \hl{nie wiem jak opisac to ze generujemy kody ze slownika, definiujemy sobie
% jakies punkty i szukamy najlepszego w danej dziedzinie - NAJlepsze ze w tej
% sekcji powinnismy to zrobic bez naszego approchu - tylko patrzcie TEN pan to juz
% robil przed nami - zrobimy podobnie}

% chlip: https://www.youtube.com/watch?v=Azf0BVBrJ0c&t=0s

% Selecting Representative Examples for Program Synthesis
% \url{https://arxiv.org/pdf/1711.03243.pdf}

% Deep Learning for Source Code Modeling and Generation:
%  Models, Applications and Challenges
%\url{https://arxiv.org/pdf/2002.05442.pdf}

% CEL: automl-zero zdefiniowalo operacje i budowalo kod
%      my zrobimy cos jak DSL na voronoi-a implementujac jako opcje rozne
%      modyfikacje (np. typ anchoru)

% Domain-specific languages (DSLs) are often used to define the parametrization
% rules and states for generating the structure of a program. DSL-based models
% create different grammar rules for common code statements (e.g., control flow,
% comments, and brackets). Compared to a general purpose programming language, the
% grammar size of a DSL is usually smaller, which makes it more efficient for
% specific code generation tasks.

% Our solution will use \textit{program template}.
...
% Come up with new modifications that can be made to the JFA (e.g. instead of communicating with your neighbors in a square shape, use a circle)
...
% FIXME: bla bla co to jest search space?

%W sekcji warianty opisujemy poszczegolne modyfikacje - niektore parametry sa
%scicle zwiazane z innymi. Aby miec spojny loss landscape nalezy rozdzieli
%przestrzen parametrow na podprzestrzenie zawierajaca tylko te gdzie kazda para w
%podprzestrzeni jest parami zalezna od siebie.
...
%such as using a circle instead of interacting with neighbours in a square pattern

%Pewne modyfikacje sa oczywiste i wynikaja z alternatywnego podejscia (zamiast anchoru kwadratowego mozna uzyc kola)
...
\hl{wariant gdzie step jest mnozony razy error z metric? w kernelu}
...
% FIXME: to nie jest wyjscie!!!
% \makecell[l]{
% 	Square-\\
% 	Special(1.44/0.96/0.17/1.63/0.86)+\\
% 	Noise}           &
